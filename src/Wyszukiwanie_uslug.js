...
function Wyszukiwanie_uslug() {
  ...
  const fetchData = () => {
    ...
    Promise.all([
      fetch("https://book-it-back.vercel.app/api/strona_wyszukiwania_kategorie").then((res) => res.json()),
      fetch("https://book-it-back.vercel.app/api/strona_wyszukiwania_miasta").then((res) => res.json()),
    ])
  ...
  const handleFilter = async (e, categoryFromHome = null) => {
    ...
    try {
      const response = await fetch("https://book-it-back.vercel.app/api/wyszukiwanie", {
        method: "POST",
        headers: {
