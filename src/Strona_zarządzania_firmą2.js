...
function Strona_zarządzania_firmą2() {
  ...
  const fetchCompanyHours = async () => {
    try {
      const response = await axios.post('book-it-back.vercel.app'+'/api/Strona_zarządzania_firmą2', { company_id });
  ...
  const fetchReservations = async (date) => {
    try {
      const response = await axios.post('book-it-back.vercel.app'+'/api/Strona_zarządzania_firmą/reservations', { company_id, date });
  ...
  const saveHours = async () => {
    try {
      ...
      await axios.post('book-it-back.vercel.app'+'/api/update_company_hours', { company_id, hours });
  ...
  const handleSaveClick = async () => {
    try {
      ...
      await axios.post('book-it-back.vercel.app'+'/api/update_reservation', { reservation: updatedReservation});
  ...
  const handleDeleteClick = async () => {
    try {
      await axios.delete('book-it-back.vercel.app'+'/api/delete_reservation', { data: { id_rezerwacji: selectedReservation.id_rezerwacji } });
  ...
  const handleDeleteAllClick = async () => {
    try {
      ...
      const formattedDate = `${year}-${month}-${day}`;
      await axios.delete('book-it-back.vercel.app'+'/api/delete_reservations_by_date', { data: { reservation_date: formattedDate } });
